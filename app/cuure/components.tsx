...
export function TimelineMonth({
  ...
  return (
    ...
    >
      {/* Month indicator */}
      <div className="absolute -left-5 top-0 w-10 h-10 rounded-full bg-[var(--accent)] flex items-center justify-center text-white font-bold text-xs">
        {month}
      </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {kpis.map((kpi, i) => (
        <motion.div
          key={kpi.label}
          className="bg-[var(--bot-bubble-bg)] border border-[var(--foreground)]/10 rounded-xl p-5 group hover:border-[var(--accent)]/50 transition-all"
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          <p className="text-xs uppercase tracking-widest opacity-40 mb-1">{kpi.category}</p>
          <p className="font-semibold mb-3">{kpi.label}</p>
          <div className="flex items-center gap-2">
            <span className="text-sm opacity-50">{kpi.current}</span>
            <motion.span
            ...
            <span className="text-lg font-bold text-[var(--accent)]">{kpi.target}</span>
          </div>
        </motion.div>
      ))}
    </div>
    ...
    <div className="space-y-3 my-4">
      {data.map((item, i) => (
        <div key={item.label} className="flex items-center gap-3">
          <span className="w-28 text-sm shrink-0">{item.label}</span>
          ...
          <span className={`text-sm font-semibold w-16 text-right ${item.highlight ? "text-[var(--accent)]" : ""}`}>
            {item.value}%
          </span>
        </div>
      ))}
...
// ============================================
// IMPACT METRIC CARD COMPONENT (NEW)
// ============================================
export function ImpactCard({
  ...
  return (
    <motion.div
      className="bg-[var(--accent)]/10 border border-[var(--accent)]/30 rounded-xl p-5 text-center"
      initial={{ opacity: 0, scale: 0.9 }}
      whileInView={{ opacity: 1, scale: 1 }}
      viewport={{ once: true }}
    ...
    >
      ...
      <div className="text-2xl md:text-3xl font-bold text-[var(--accent)] mb-1">{value}</div>
      <p className="text-sm opacity-60">{label}</p>
    </motion.div>
  );
}
...
// ============================================
// UNIT ECONOMICS TABLE (NEW)
// ============================================
export function UnitEconomicsCompare({
  title1,
  title2,
  metrics,
}: {
  title1: string;
  title2: string;
  metrics: { label: string; value1: string; value2: string }[];
}) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <motion.div
        className="bg-[var(--bot-bubble-bg)] border border-[var(--foreground)]/10 rounded-xl p-5"
        initial={{ opacity: 0, x: -20 }}
        whileInView={{ opacity: 1, x: 0 }}
        viewport={{ once: true }}
      >
        <h5 className="font-semibold mb-4 opacity-60">{title1}</h5>
        <div className="space-y-3">
          {metrics.map((m) => (
            <div key={m.label} className="flex justify-between text-sm">
              <span className="opacity-70">{m.label}</span>
              <span className="font-semibold">{m.value1}</span>
            </div>
          ))}
        </div>
      </motion.div>
      <motion.div
        className="bg-[var(--accent)]/10 border border-[var(--accent)]/30 rounded-xl p-5"
        initial={{ opacity: 0, x: 20 }}
        whileInView={{ opacity: 1, x: 0 }}
        viewport={{ once: true }}
      >
        <h5 className="font-semibold mb-4 text-[var(--accent)]">{title2}</h5>
        <div className="space-y-3">
          {metrics.map((m) => (
            <div key={m.label} className="flex justify-between text-sm">
              <span className="opacity-70">{m.label}</span>
              <span className="font-bold text-[var(--accent)]">{m.value2}</span>
            </div>
          ))}
        </div>
      </motion.div>
