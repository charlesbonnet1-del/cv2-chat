...
export function ComparisonBar({
  ...
  return (
    <div ref={ref} className="flex items-center gap-4 py-2">
      <span className="w-32 text-sm opacity-60">{label}</span>
      <div className="flex-1 flex items-center gap-2">
        <span className="text-sm opacity-40 line-through">{before}</span>
        <motion.div
          className="flex-1 h-2 bg-[var(--foreground)]/10 rounded-full overflow-hidden"
          initial={{ opacity: 0 }}
        ...
        <span className="text-sm font-semibold text-[var(--accent)]">{after}</span>
      </div>
    </div>
  );
}
...
export function CompetitorTable({
  ...
  const headers = ["Origine", "Cryptos", "Frais", "PSAN", "Support FR", "Gestion pilotée", "Trustpilot"];
  const dataKeys = ["origin", "cryptos", "fees", "psan", "support", "managed", "trustpilot"];
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {competitors.map((comp, i) => (
            ...
            >
              <td className={`p-3 font-semibold ${comp.highlight ? "text-[var(--accent)]" : ""}`}>
                {comp.name}
              </td>
              {dataKeys.map((key) => (
